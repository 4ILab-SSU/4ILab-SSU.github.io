(() => {
  ...
  tiles.on('tileerror', () => { status.textContent = t('지도 배경을 불러오지 못했습니다. 장소 목록의 앨범 링크는 계속 이용할 수 있습니다.'); });
  ...
  Object.entries(places).forEach(([id, place]) => {
    ...
    const list = node('ul');
    matching.forEach(album => { const item = node('li'); const a = node('a', `${album.date} · ${album.title} (${album.images.length} ${t('장')})`); a.href = `#album-${album.date}`; item.append(a); list.append(item); });
    popup.append(list);
    const marker = L.marker(point, {
      albumCount: matching.length, title: `${place.name} · ${matching.length} ${t('개 앨범')}`, alt: `${place.name} · ${t('앨범 열기')}`,
